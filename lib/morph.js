...
export function morph (slot, newChildren, ctx, render) {
  ...
  for (let i = 0, len = newChildren.length; i < len; i++) {
    ...
    if (newChild != null) {
      ...
      if (!oldChild) {
        const next = find(slot.siblings, slot.index + 1)
        if (next) {
          next.before(...args)
        } else {
          // Handle last child of fragment, will fail if all children are null
          const prev = find(slot.siblings, slot.index - 1, -1)
          if (prev) prev.after(...args)
          else slot.parent.append(...args)
        }
      } else if (newChild !== oldChild) {
        const next = find([oldChild])
        next.before(...args)
      }
    }

...
function mountSlot (slot, newChildren, ctx) {
  ...
  for (; i < length; i++) {
    ...
    newChildren[i] = mountChild(newChildren[i], i, slot, ctx)
  }
...
function mountChild (newChild, index, slot, ctx) {
      ...
      const { value, child } = newChild.initialize(
        ...
        function onupdate (value, ctx, afterupdate) {
          if (!value) {
            ...
            if (retry) {
              slot.children[index] = mountChild(value, index, slot, ctx)
              // Override cache set to nested child context
              cache.set(slot.children[index], ctxPublic)
            } else {
  ...
  if (children.length && newChild != null) {
    if (newChild === INTERMEDIATE) {
      newChild = children.shift()
    } else {
...
function mountTemplate (template, nodes, parent, values, ctx) {
  ...
  for (let node = nodes.shift(); node; node = nodes.shift()) {
    if (isPlaceholderNode) {
      ...
      if (!getChildren(slot).includes(node)) remove(node)

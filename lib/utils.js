...
export const INTERMEDIATE = Symbol('INTERMEDIATE')
...
export function toNode (value) {
  if (value == null || value === INTERMEDIATE) return null
...
/**
 * Find first non-null value in array from index
 * @param {Node[]} nodes List of nodes to search
 * @param {number} [index=0] Index of node to start search from
 * @param {number} [direction=1] Direction to search in, 1/-1
 * @returns {Node | void}
 */
export function find (nodes, index = 0, direction = 1) {
  for (let i = index, len = nodes.length; i < len && i >= 0; i += direction) {
    let next = nodes[i]
    if (next instanceof Slot) {
      next = find(next.siblings, next.index + 1, direction)
    }
    if (next instanceof Fragment) next = find(next.children, 0, direction)
    if (next) return next
  }
}
